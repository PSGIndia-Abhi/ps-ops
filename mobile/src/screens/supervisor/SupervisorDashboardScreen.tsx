...
import type { AuthenticatedStackParamList, SupervisorTabParamList } from '../../navigation/types';

type Nav = NativeStackNavigationProp<AuthenticatedStackParamList & SupervisorTabParamList>;
...
export function SupervisorDashboardScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    setError(null);
    try {
      const [summaryData, jobs, teamData] = await Promise.all([
      ...
      // Shift status is a nice-to-have overlay on the roster, not core to it -
      // a failure here shouldn't block the rest of the dashboard.
      try {
        const shifts = await teamsApi.getShiftStatusByTechnician();
        const onlineIds = new Set(shifts.online.map((t) => String(t.technician_id)));
        setOnShiftCount(teamData.filter((m) => onlineIds.has(String(m.id))).length);
      } catch {
        setOnShiftCount(null);
      }
    } catch (err) {
  ...
  return (
    <View style={styles.flex}>
      ...
      <ScreenContainer onRefresh={() => load(true)} refreshing={refreshing}>
        ...
        <Text style={styles.subtitle}>Here's how your team is doing today.</Text>

        {!!error && <Banner message={error} variant="error" />}

        {/* Today's team workload */}
        ...
        <View style={styles.sectionHeaderRow}>
          ...
          <Text style={styles.viewAll} onPress={() => navigation.navigate('Team')}>
            View all →
          </Text>
        </View>
        {!team && !error ? (
          ...
          <View style={styles.snapshotCard}>
            <Text style={styles.snapshotTotal}>
              {team.length} {team.length === 1 ? 'Technician' : 'Technicians'}
            </Text>
            <View style={styles.snapshotRow}>
              ...
              <View style={styles.snapshotItem}>
                ...
                <Text style={styles.snapshotText}>
                  {onShiftCount !== null ? team.length - onShiftCount : '–'} Off shift
                </Text>
              </View>
            </View>
          </View>
        ...
        {today === null && !error ? (
          <Skeleton height={130} radius={16} style={styles.skeletonGap} />
          ...
          <View style={styles.scheduleCard}>
            {scheduleJobs.map((job, index) => (
              <ScheduleRow
                ...
                jobType={job.title}
                technicianName={job.team[0]?.name}
                status={job.status}
        ...
        {/* Quick access */}
        <Text style={[styles.sectionTitle, styles.sectionTitleSpaced]}>Quick access</Text>
